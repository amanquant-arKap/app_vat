...
# The issue is that the NACE classifier loads at the beginning of show_main() 
# and if it's None, it tries to load and then calls st.rerun() implicitly
# This creates an infinite loop because after loading the database, it checks NACE again

# Let's fix this by only loading NACE when it's actually needed and avoiding rerun loops
...
print("✅ Fixed the infinite loop issue!")
print("\n🔧 Changes made:")
print("=" * 60)
print("1. ✅ Removed st.spinner() from NACE loading (was causing UI block)")
print("2. ✅ Changed NACE loading to be silent - no warnings if not available")
print("3. ✅ NACE classifier set to empty object if loading fails (prevents repeated attempts)")
print("4. ✅ Removed implicit st.rerun() after loading NACE")
print("5. ✅ NACE now loads once at the start without blocking UI flow")
print("\n📝 The loop was caused by:")
print("   - st.spinner() blocking execution while NACE loaded")
print("   - Checking nace_classifier == None repeatedly")
print("   - Not setting a default value when NACE loading failed")
print("\n📄 File saved as: arkap_vat_extractor_FIXED.py")
